...
* `org.mapstruct.Qualifier`
    ** == custom annotation | mapping method /
        *** -- is included as -- used mapper
        *** -- can be referred to --
            **** bean property mapping
                ***** == `@BeanMapping#qualifiedBy`
                ***** `@Named` & `qualifiedByName`
                    ****** 👁️recommended alternative 👁️
                    ****** includes `@Qualifier`
                    ****** allows
                        ******* naming a Mapper or
                        ******* naming a mapping method -- via -- its value
            **** iterable mapping
            **** map mapping
    ** uses
        *** | mapping methods / SAME method signature & different behavior
            **** several can be | 1 mapping method
* _Example1:_ | map mapping, hand-written methods / ALMOST SAME signature, except to name
...
.Mapper causing an ambiguous mapping method error, because SAME arguments and returns -> Mapper finds 2 qualifying methods
====
[source, java, linenums]
[subs="verbatim,attributes"]
----
...
@Qualifier
@Target(ElementType.TYPE)       // type level
...
.Declaring qualifier types / mapping methods
====
[source, java, linenums]
[subs="verbatim,attributes"]
----
...
* 👁️take in account that 👁️
    ** retention policy == `@Retention(RetentionPolicy.CLASS)`
    ** class / method -- annotated with a -- qualifier -> ONLY qualify mappings / have the `qualifiedBy` element

* _Example2:_ Previous example -- via -- `@Named`
...
* TODO:
Please note that the `Mapping#defaultValue` is in essence a `String`, which needs to be converted to the `Mapping#target`.
Providing a `Mapping#qualifiedByName` or `Mapping#qualifiedBy` will force MapStruct to use that method.
If you want different behavior for the `Mapping#defaultValue`, then please provide an appropriate mapping method.
This mapping method needs to transforms a `String` into the desired type of `Mapping#target` and also be annotated
so that it can be found by the `Mapping#qualifiedByName` or `Mapping#qualifiedBy`.
