...
* `Mapping#defaultValue`
    ** == `String` / -- needs to be converted to the -- `Mapping#target`
    ** if you ALSO indicated `Mapping#qualifiedByName` or `Mapping#qualifiedBy` -> `defaultValue` is passed as argument
...
public interface MovieMapper {
     ...
     // if category is null -> `CategoryToString( Enum.valueOf( Category.class, "DEFAULT" ) )` is called & the result set to the category field
...
public interface MovieMapper {
     ...
     // if category is null -> `defaultValueForQualifier( "Unknown" )` is called & the result set to the category field
     @Mapping( target = "category", qualifiedByName = "CategoryToString", defaultValue = "Unknown" )
     GermanRelease toGerman( OriginalRelease movies );

     // default method  == modified default
     @Named("CategoryToString")
     default String defaultValueForQualifier(Category cat) {
         // some mapping logic
     }
